...
function clickBtn() {
  ...
  randomNumArray.forEach((num, i) => {
    if(num <= 10 ){
      span[i].style.backgroundColor = "#fbc400" //노란색
    }else if (num <= 20) {
      span[i].style.backgroundColor = "#69c8f2" //파란색
    }else if (num <= 30 ) {
      span[i].style.backgroundColor = "#ff7272" //빨간색
    }else if (num <= 40 ) {
      span[i].style.backgroundColor = "#aaa" //회색
    }else {
      span[i].style.backgroundColor = "#b0d840" //초록색
    }
  })
